...
if [[ ! -f "$ZOO_CONF_DIR/zoo.cfg" ]]; then
    ...
    if [[ -z $ZOO_SERVERS ]]; then
        if [[ ! -z $ZOO_AWS_SERVICE ]]; then
            servers=$(aws servicediscovery list-instances --service-id=$ZOO_AWS_SERVICE --query Instances[*].Attributes.AWS_INSTANCE_IPV4 --output text)
            counter=1
